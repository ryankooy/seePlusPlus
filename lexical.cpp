...
template <class F>
void with_IO(string source, string dest, F&& f)
{
  string input;
  if (source == "stdin")
    getline(cin, input);
  else
    input = file_to_string(source);
  string output = invoke(forward<F>(f), input);
  if (dest == "stdout")
    cout << output;
  else
    string_to_file(dest, output);
}
...
private:
  ...
  Token follow(char expected, TokenName ifyes, TokenName ifno)
  {
    if (s.next() == expected)
      ...
      return make_token(ifno);
  }

  Token divide_or_comment()
  {
    if (s.next() != '*')
      return make_token(TokenName::OP_DIVIDE);
    while (s.next() != '\0') {
      if (s.peek() == '*' && s.next() == '/') {
        s.advance();
        return next_token();
    ...
    if (n == '\\')
      switch (s.next()) {
        ...
        case '\\':
          ...
          break;
        default:
          return error("Unknown escape sequence \\", s.peek());
      }
    if (s.next() != '\'')
      return error("Multi-character constant");
    s.advance();
  ...
  Token string_lit()
  {
    ...
    while (s.next() != '"')
      switch (s.peek()) {
        ...
        case '\0':
          return error( "End-of-file while scanning string literal."
  ...
  static inline bool is_id_end(char c)
  {
  ...
  Token identifier()
  {
    ...
    while (is_id_end(s.next()))
      text += s.peek();
    auto i = keywords.find(text);
    if (i != keywords.end())
      return make_token(i->second);
  ...
  Token integer_lit()
  {
    ...
    if (r.ec == errc::result_out_of_range)
      return error("Number exceeds maximum value.");
    return make_token(TokenName::INTEGER, n);
  }
};
